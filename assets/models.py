...
class CPU(models.Model):
    """CPU组件"""
    asset = models.OneToOneField('Asset')
    cpu_model = models.CharField('CPU型号', max_length=128, blank=True,default=True)
...
class RAM(models.Model):
    """内存组件"""
    asset = models.ForeignKey('Asset')   # 只能通过外键关联Asset。 否则不能同时管理服务器、网络设备等
...
class NIC(models.Model):
    """网卡组件"""
    asset = models.ForeignKey('Asset')      #注意外键
    ...
    class Meta:
        ...
        unique_together = ('asset', 'model', 'mac')      # 资产、型号、mac必须联合唯一，防止虚拟机的特殊情况发生错误
...
class EventLog(models.Model):
    ...
    user = models.ForeignKey(User, blank=True, null=True, verbose_name='事件执行人', on_delete=models.SET_NULL) #自动更新资产时没有执行人
    memo = models.TextField('备注', blank=True, null=True)
...
class NewAssetApprovalZone(models.Model):
    """新资产待审批区"""
    sn = models.CharField('资产SN号', max_length=128, unique=True) #次字段必填
    asset_type_choice = (
        ('server', '服务器'),
        ('networkdevice', '网络设备'),
        ('storagedevice', '存储设备'),
