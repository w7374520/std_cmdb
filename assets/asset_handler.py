...
class ApproveAsset:
    ...
    def _server_upline(self):
        ...
        asset = self._create_asset()        # 创建一条资产并返回资产对象。注意要和待审批去的资产区分开
        try:
    ...
    def _create_manufacturer(self, asset):
        ...
        :return:
        """
        # 判断厂商数据是否存在。如果存在，看看数据库里面是否有已经有该厂商，再决定是获取还是创建。
    ...
    def _create_CPU(self, asset):
        ...
        实际情况下你可能还要数据的完整性、合法性、数据类型进行检测，
        根据不同检测情况，是被动接收，还是打回去要求重新收集，请自行决定。
        这里业务逻辑非常复杂，不可能面面俱到。
        :param asset:
        :return:
        """
    ...
    def _create_RAM(self, asset):
        ...
        for ram_dict in ram_list:
            if not ram_dict.get('slot'):
                raise ValueError("未知的内存插槽!")   # 使用虚拟机的时候， 可能无法获取内存的插槽，需要你修改改此处的逻辑
            ram = models.RAM()
            ram.asset  = asset
    ...
    def _create_disk(self, asset):
        """
        存储设备种类，还有raid情况，需要根据实际情况具体解决。
        这里只以简单的sata硬盘为例子，这可能有多块硬盘
        :param asset:
        :return:
        """
        ...
        for nic_dict in nic_list:
            ...
            if nic_dict.get('net_mask'):
                if len(nic_dict.get('net_mask')) > 0:
                    nic.get_mask = nic_dict.get('net_mask')[0]
            nic.save()

    def _delete_original_asset(self):
        """
        这里的逻辑上已经审批上线的资产，就从待审批区删除。
