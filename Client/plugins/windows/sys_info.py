...
"""
本模块基于windows操作系统，依赖wmi和win32com库，需要提前使用pip进行安装，或者下载安装包手动安装
pip install wmi
...
class Win32Info(object):
    ...
    def get_ram_info(self):
        ...
        for item in ram_collections:     # 主机中存在很多根内存， 要循环所有的内存数据
            ...
            data.append(item_data)  #将每条内存信息，添加到一个列表里

        return {"ram" : data}   # 再对data列表封装一层，返回一个字典，方便上级方式的调用
    ...
    def get_disk_info(self):
        ...
        """
        data = []
        for disk in self.wmi_obj.Win32_DiskDrive():     #获取每块硬盘的信息
            item_data = dict()
