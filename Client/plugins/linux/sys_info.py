...
# -*- coding: utf-8 -*-
"""
原来的get_nic_info函数无法获取centos7的数据，ubutun也许可以使用，因为没有ubuntu系统无法测试
自我实现get_centos7_nic_info函数，centos7测试通过
"""
...
def get_disk_info():
    """
    获取存储信息
    本脚本之针对ubuntu中使用的sda，且只有一块硬盘的情况，具体查看硬盘信息的命令，请根据实际情况实际调整
...
def get_centos7_nic_info():
    """
    获取Centos7.X网卡信息
    :return:
    """
    raw_data = subprocess.Popen("ip address", stdout=subprocess.PIPE, shell=True)
    raw_data = raw_data.stdout.read().decode().split("\n")
    nic_dic = dict()

    for line in raw_data:

        if re.findall("(\d):\s\w", line):
           num=re.findall("(\d):\s\w", line)[0]
           #print line
           #print(num)
        ...
        if re.findall("\d: (\S+): <", line):
           #print(line)
           nic_name = re.findall("\d: (\S+): <", line)[0]
           #print(nic_name)
        ...
        elif re.findall("link/ether (\S+) brd", line):
           mac_addr = re.findall(" link/ether (\S+) brd", line)[0]
           #print(mac_addr)
           if re.findall("brd ff:ff:ff:ff:ff:ff",line):
                #print(line)
                nic_dic[num] = {'name': nic_name,
                              'mac': mac_addr,
                              'net_mask': netmask,
                              'network': network,
                              'bonding': num,
        ...
        elif re.findall("inet (\S+) brd", line):
           raw_ip_addr = re.findall("inet (\S+) brd", line)[0]
           line = line.split()
           ip_addr = line[1]
           network = line[3]
           netmask = line[1].split("/")[1]
        
        elif (re.findall("valid_lft forever preferred_lft forever", line)):
           nic_dic[num] = {'name': nic_name,
                         'mac': mac_addr,
                         'net_mask': netmask,
                         'network': network,
